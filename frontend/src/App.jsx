...
function SwipeCard({ match, onSwipe, getInitials, isTop }) {
...
function SwipeInterface({ matches, user, setPage, setSelectedMatch, token }) {
      ...
      {hasMatches ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 20px", maxWidth: 600, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <button className="btn-secondary" style={{ padding: "12px 24px" }} onClick={() => setPage("dashboard")}>
              Back to List View
            </button>
          </div>
        </div>
...
function DashboardPage({ user, setPage, setSelectedMatch, token }) {
  ...
  return (
    <div style={{ minHeight: "100vh", paddingTop: 80, padding: "100px 24px 60px", position: "relative", overflow: "hidden" }}>
      ...
      {viewMode === "swipe" && matches.length > 0 && (
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        {viewMode === "swipe" && !loading && matches.length > 0 ? (
          <SwipeInterface matches={matches} user={user} setPage={setPage} setSelectedMatch={setSelectedMatch} token={token} />
        ) : (
          <>
                        ...
                        {isExpanded && (
                          <div className="anim-fade-in" style={{ padding: "0 24px 24px", borderTop: `1px solid ${C.border}`, paddingTop: 20 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                              {[{ label: "Sleep", value: match.sleep }, { label: "Cleanliness", value: match.clean }, { label: "Area", value: match.area }, { label: "Budget", value: match.budget }].map((d, j) => <div key={j} style={{ background: C.surfaceLight, borderRadius: 10, padding: "12px 16px" }}><div style={{ fontSize: 11, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>{d.label}</div><div style={{ fontSize: 14, fontWeight: 500 }}>{d.value}</div></div>)}
                            </div>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: C.surface, border: `2px solid ${C.border}`, borderRadius: 24, maxWidth: 500, width: "100%", maxHeight: "90vh", overflowY: "auto", position: "relative" }}>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {[
                    { label: "Sleep", value: profileModalData.sleep || profileModalData.sleepSchedule, icon: "🌙" },
                    { label: "Cleanliness", value: profileModalData.clean || profileModalData.cleanliness, icon: "✨" },
                    { label: "Area", value: profileModalData.area || profileModalData.location, icon: "📍" },
                    { label: "Budget", value: profileModalData.budget, icon: "💰" }
                  ].filter(d => d.value).map((d, j) => (
                    <div key={j} style={{ background: C.surfaceLight, borderRadius: 12, padding: 12 }}>
